...
if __name__ == '__main__':
    ...
    parser.add_argument("--frozen_model_filename", default="built_graph/yolov2_logos.pb", type=str, help="Frozen model file to import, default built_graph/yolov2_logos.pb")
    parser.add_argument("--batch_size", default=8, type=int, help="Batch size, default 8")
    parser.add_argument("--input_path", default="images/", type=str, help="Path of the images to predict, default images/")
    ...
    try:
        os.mkdir(os.path.join(inp_path, 'out'))
    except:
        pass
    ...
    batch = min(args.batch_size, len(all_inps))
        ...
        for j in range(n_batch):
            ...
            inp_feed = pool.map(lambda inp: (
                np.expand_dims(preprocess(
                    os.path.join(inp_path, inp), meta), 0)), this_batch)

            feed_dict = {x : np.concatenate(inp_feed, 0)}    
            y_out = sess.run(y, feed_dict)
            # for idx, y in enumerate(y_out):
            #     postprocess(y, os.path.join(inp_path, this_batch[idx]), meta)
            pool.map(lambda p: (lambda i, prediction:
                postprocess(
                prediction, os.path.join(inp_path, this_batch[i]), meta))(*p),
                enumerate(y_out))
            print("\tBatch #" + str(j) + " is done.")
    print("All done.")